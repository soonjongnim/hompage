...
( function( window, $ )
{
    ...
    function Wysiwyg( element, userOptions ) {
     ...
     Wysiwyg.prototype.imageUpload = function(dataUrl, fileType){
    	...
    	formData.append("folderName", "BBS");
    	return $.ajax({
    		type:"post",
    		url:"/com/imageUpload.do",
    		data:formData,
    		async:false,
	        cache:false,
	        contentType:false,
        ...
        $.each( files, function( idx, fileInfo ) {
            if ( /^image\//.test( fileInfo.type ) ) {
                $.when( self.readFileIntoDataUrl( fileInfo ) ).done( function( dataUrl ) {
                	self.imageUpload(dataUrl, fileInfo.type).done(function(data){
                		self.execCommand( "insertimage", data.imagePath, editor, options, toolbarBtnSelector );
